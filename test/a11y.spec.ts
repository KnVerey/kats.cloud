...
test.describe('Accessibility (axe-core)', () => {
  ...
  const states: Array<{ name: string; cmd: (page: Page) => Promise<void> }> = [
    {
      name: 'Biomed (dark)',
      async cmd() {
        /* default dark */
    ...
    },
    {
      ...
      async cmd(page) {
        await page.click('#themeToggle');
        await page.click('#tab-software');
      },
    },
    {
      name: 'Linguistics (light)',
      async cmd(page) {
        await page.click('#themeToggle');
        await page.click('#tab-linguistics');
      },
    },
  ];
  ...
  for (const s of states) {
    test(`axe: ${s.name}`, async ({ page }) => {
      ...
      expect(violations, `${s.name} should have no serious/critical axe violations`).toHaveLength(
        0
      );
    });
  }
